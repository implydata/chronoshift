...
describe("Duration", () => {
  ...
  describe("errors", () => {
    it("throws error if invalid duration", () => {
      ...
      expect(() => Duration.fromJS("P0W").shift(new Date(), TZ_LA)).toThrow("Duration can not have empty weeks");

      expect(() => Duration.fromJS("P0Y0MT0H0M0S").shift(new Date(), TZ_LA)).toThrow("Duration can not be empty");
    });
  ...
  describe("construct from span", () => {
    it("parses days over DST", () => {
      expect(new Duration(
        ...
        TZ_LA
      ).toString()).toEqual("P7D");

      expect(new Duration(
        new Date("2012-10-29T00:00:00-07:00"),
        new Date("2012-11-12T00:00:00-08:00"),
        TZ_LA
      ).toString()).toEqual("P14D");
    });
  ...
  describe("#floor", () => {
    it("throws error if complex duration", () => {
      expect(() => Duration.fromJS("P1Y2D").floor(new Date(), TZ_LA)).toThrow("Can not floor on a complex duration");

      expect(() => Duration.fromJS("P3DT15H").floor(new Date(), TZ_LA)).toThrow("Can not floor on a complex duration");

      expect(() => Duration.fromJS("PT5H").floor(new Date(), TZ_LA)).toThrow("Can not floor on a hour duration that does not divide into 24");
    });
    ...
    it("works for P1W", () => {
      ...
      expect(p1w.floor(new Date("2013-10-03T01:02:03.456-07:00"), TZ_LA))
        .toEqual(new Date("2013-09-30T00:00:00.000-07:00"));
    });
  ...
  describe("#shift", () => {
    it("works for weeks", () => {
      ...
      expect(p1w.shift(new Date("2012-10-29T00:00:00-07:00"), TZ_LA)).toEqual(new Date("2012-11-05T00:00:00-08:00"));

      p1w = Duration.fromJS("P1W");
      expect(p1w.shift(new Date("2012-10-29T00:00:00-07:00"), TZ_LA, 2)).toEqual(new Date("2012-11-12T00:00:00-08:00"));

      p2w = Duration.fromJS("P2W");
      expect(p2w.shift(new Date("2012-10-29T05:16:17-07:00"), TZ_LA)).toEqual(new Date("2012-11-12T05:16:17-08:00"));
    });
  ...
  describe("#materialize", () => {
    it("works for weeks", () => {
      ...
      expect(p1w.materialize(new Date("2012-10-29T00:00:00-07:00"), new Date("2012-12-01T00:00:00-08:00"), TZ_LA, 2)).toEqual([
        new Date('2012-10-29T07:00:00.000Z'),
        new Date('2012-11-12T08:00:00.000Z'),
        new Date('2012-11-26T08:00:00.000Z')
      ]);
